...
func TestInterpreter(t *testing.T) {
	t.Run("Test type checking float64", func(t *testing.T) {
		p := &Interpreter{}
		v := float64(3.4)
		err := p.checkType(&Token{}, v, []reflect.Kind{reflect.Float64})
		assert.NoError(t, err)
	})
	t.Run("Test type checking bool", func(t *testing.T) {
		p := &Interpreter{}
		v := true
		err := p.checkType(&Token{}, v, []reflect.Kind{reflect.Float64})
		fmt.Println(err)
